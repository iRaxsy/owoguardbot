...
client.ownerst = ["719692235775541318","194686637597392898","","","","","","","","","","","","","","","","","","","","",""]; // KURUCU
client.evulate = []
...
client.botAccounts = ["1140629279403692144","1141041190704263169","1140629279403692144","1142067329665081375","1141041190704263169","770299344250339349","868528396261543936","689766089567109158","899899858981371935","530082442967646230","424606447867789312","499595256270946326","490039330388180992","851436490415931422","681137419663441933","408785106942164992","825299120364322826","282859044593598464","519287796549156864","557628352828014614","1117439882080563360"] // Güvenli bot ID
client.botroles = ["1140629279403692144","1141041190704263169","1140629279403692144","1142067329665081375","1141041190704263169","770299344250339349","868528396261543936","689766089567109158","899899858981371935","530082442967646230","424606447867789312","499595256270946326","490039330388180992","851436490415931422","681137419663441933","408785106942164992","825299120364322826","282859044593598464","519287796549156864","557628352828014614","1117439882080563360"] // YLER GİDİNCE YETKİSİ GİTMİYECEK BOTLARIN ID
let kanal = "1142065488403365978" // LOG KANAL
let ustKanal = "1142065488403365978" // YETKİSİ YETMEYİNCE BLABLA

client.on("ready", () => {
    client.user.setActivity("OwO World <3");
...
client.on("guildUnavailable", async (guild) => {
    ...
    client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> Sunucu kullanılamaz hale geldiği için koruma amacıyla yetkileri kapadım!`)
});
...
client.on("guildBanAdd", async (guild, member) => {
    await guild.fetchAuditLogs({
        ...
        if (banLimit == 3) {
            client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> <@${yapan.id}> | (\`${yapan.id}\`) kişisi <@${hedef.id}> | (\`${hedef.id}\`) kişisini sağ tık yöntemiyle yasakladığı için sunucudan yasaklandı!`)
            let arr = ["ADMINISTRATOR", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "VIEW_AUDIT_LOG"]
            guild.roles.cache.filter(a => arr.some(x => a.permissions.has(x)) == true && guild.members.cache.get(client.user.id).roles.highest.rawPosition > a.rawPosition && !client.botroles.includes(a.id)).map(huh => {
                //client.roleBackup.set(huh.id, huh.permissions.bitfield)
                huh.setPermissions(0)
...
client.on("guildUpdate", async (oldGuild, newGuild) => {
    await newGuild.fetchAuditLogs({
        ...
        if (oldGuild.name !== newGuild.name) {
            newGuild.setName("OwO - Turkey")
            newGuild.members.ban(yapan.id, {
                reason: "Sunucu ismi değiştirmek."
            })
            client.blackList.push(yapan.id)
            client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> <@${yapan.id}> | (\`${yapan.id}\`) kişisi tarafından sunucu ismi değiştirildi. Kişi banlandı, Sunucu ismi eski haline çevirildi.`)
        }
    ...
    await newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.name !== newChannel.name) {
            ...
            if (limitOfChannel.length == 2) {
                ...
                client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> <@${yapan.id}> | (\`${yapan.id}\`) kişisi ${limitOfChannel.length} kanalın ismini değiştirdiği için yasaklandı.Değiştirmeye çalıştığı kanal isimleri aşağıda belirtilmiştir.\`\`\`${mapped.join("\n")}\`\`\``)
...
client.on("roleUpdate", async (oldRole, newRole) => {
    await newRole.guild.fetchAuditLogs({
        ...
        if (oldRole.name !== newRole.name) {
            client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> (\`${yapan.id}\`) | <@${yapan.id}> kişisi ${oldRole.name} rolün ismini değiştirdiği için sunucudan yasaklandı.`)
...
client.on("channelCreate", async channel => {
    await channel.guild
        ...
        .then(async audit => {
            ...
            if (limit.length == 3) {
                client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> <@${yapan.id}> | (\`${yapan.id}\`) kişisi toplam 3 kanal açtığı için sunucudan yasaklandı kanallar siliniyor. Açtığı kanallar \`\`\`${limit.map(x => channel.guild.channels.cache.get(x).name).join("\n")}\`\`\``);
    ...
    newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.permissionOverwrites !== newChannel.permissionOverwrites) {
            ...
            if (everyonePermission.includes("MENTION_EVERYONE") || everyonePermission.includes("MANAGE_CHANNELS")) {
                ...
                client.blackList.push(yapan.id)
                client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> <@${yapan.id}> | (\`${yapan.id}\`) kişisi ${newChannel.name} kanalının everyone izinlerine gereksiz izin tanıdığı için kullanıcı yasaklandı.`);
...
client.on("guildBanRemove", async (guild, member) => {
    ...
    await guild.fetchAuditLogs({
        ...
        if (client.ownerst.includes(yapan.id)) return
        if (Date.now() - ayar.createdTimestamp > 5000) return;
        client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> <@${yapan.id}> | (\`${yapan.id}\`) kişisi daha önceden guard tarafından ban yiyen <@${member.id}> | (\`${member.id}\`) kişisinin yasağını kaldırdığı için banlandı !`)
        await guild.members.ban(yapan.id, {
...
client.on("channelUpdate", async (oldChannel, newChannel) => {
    newChannel.guild.fetchAuditLogs({
        ...
        if (oldChannel.permissionOverwrites !== newChannel.permissionOverwrites) {
            ...
            client.blackList.push(yapan.id)
            client.channels.cache.get(kanal).send(`<a:hayir:1091391891557339146> <@${yapan.id}> kişisi ${newChannel.name} kanalına gereksiz izin tanıdığı için kullanıcı yasaklandı.`);
        }
    });
});
...
client.on("message", async message => {
    ...
    if (message.author.id !== "719692235775541318") return
...
client.login("MTE0MjA2NzMyOTY2NTA4MTM3NQ.GXGknV.JaohIQ0zJHeSq0iQ0YQLt5ZM1mJGe0cW8Q7N4E")
